...
public class VendedorController {
	...
	public ResponseEntity<Vendedor> listarPorId(@PathVariable("id") Integer id) {
		Vendedor cli = service.leerPorId(id);
		if (cli.getId_Vendedor() == null) {
			throw new ModeloNotFoundException("ID NO ENCONTRADO" + id);
			
		}
		return new ResponseEntity<Vendedor>(cli, HttpStatus.OK);
		
	}
	
	@GetMapping("/pageable")
	public ResponseEntity<Page<Vendedor>> listarPageable(Pageable pageable) {
		Page<Vendedor> vendedor = service.listarPageable(pageable);
		return new ResponseEntity<Page<Vendedor>>(vendedor, HttpStatus.OK);
	}
	
	 @PostMapping 
	 public ResponseEntity<Object> registrar(@Valid @RequestBody Vendedor vendedor){
		 Vendedor cli = service.registrar(vendedor);
	 ...
	 @PutMapping
		public ResponseEntity<Vendedor> modificar(@Valid @RequestBody Vendedor vendedor) {
		 Vendedor cli = service.modificar(vendedor);
			return new ResponseEntity<Vendedor>(cli, HttpStatus.OK);
		}
		@DeleteMapping("/{id}")
