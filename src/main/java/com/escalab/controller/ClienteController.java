...
public class ClienteController {
	...
	public ResponseEntity<Page<Cliente>> listarPageable(Pageable pageable) {
	 ...
	 @PostMapping 
	 public ResponseEntity<Object> registrar(@Valid @RequestBody Cliente clientes){
		 Cliente cli = service.registrar(clientes);
		 
		 URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(clientes.getId_Cliente()).toUri();
		 return ResponseEntity.created(location).build();
	 }
	 
	 @PutMapping
		public ResponseEntity<Cliente> modificar(@Valid @RequestBody Cliente clientes) {
		 Cliente cli = service.modificar(clientes);
			return new ResponseEntity<Cliente>(cli, HttpStatus.OK);
		}
